...
const DEFAULT = { checked: [], notes: {}, customStops: {}, syncedPhotos: {}, hiddenStops: [], stopOrder: {} };
...
export function useSyncState() {
      ...
      .then((remote) => {
        if (remote && !remote.error) {
          const merged = {
            ...
            syncedPhotos: { ...local.syncedPhotos, ...(remote.syncedPhotos || {}) },
            hiddenStops: [...new Set([...(local.hiddenStops || []), ...(remote.hiddenStops || [])])],
            stopOrder: { ...(local.stopOrder || {}), ...(remote.stopOrder || {}) },
          };
          setState(merged);
          saveLocal(merged);
        }
  ...
  const hideStop = useCallback((stopId) => {
    setState((prev) => {
      const next = {
        ...prev,
        hiddenStops: [...(prev.hiddenStops || []), stopId],
      };
      pushToServer(next);
      return next;
    });
  ...
  const setStopOrder = useCallback((dayIndex, orderedIds) => {
    setState((prev) => {
      const next = {
        ...prev,
        stopOrder: { ...(prev.stopOrder || {}), [dayIndex]: orderedIds },
      };
      pushToServer(next);
      return next;
    });

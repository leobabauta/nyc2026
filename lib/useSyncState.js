...
export function useSyncState() {
  ...
  const addCustomStop = useCallback((dayIndex, name, location) => {
    ...
    setState((prev) => {
      ...
      const next = {
        ...
        customStops: {
          ...prev.customStops,
          [dayIndex]: [...dayStops, { id, name, location: location || "" }],
        },
      };
      pushToServer(next);
      return next;
  ...
  const moveCustomStop = useCallback((dayIndex, stopId, direction) => {
    setState((prev) => {
      // Key could be string or number depending on JSON round-trip
      const key = Object.keys(prev.customStops).find((k) => String(k) === String(dayIndex));
      if (!key) return prev;
      const dayStops = [...(prev.customStops[key] || [])];
      const idx = dayStops.findIndex((s) => s.id === stopId);
      if (idx < 0) return prev;
      const newIdx = idx + direction;
      if (newIdx < 0 || newIdx >= dayStops.length) return prev;
      const temp = dayStops[idx];
      dayStops[idx] = dayStops[newIdx];
      dayStops[newIdx] = temp;
      const next = {
        ...prev,
